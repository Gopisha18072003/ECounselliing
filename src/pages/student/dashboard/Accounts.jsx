...
export default function Accounts() {
  ...
  const resetPasswordAction = async (prev, formData) => {
    ...
    const response = await resetPasswordStudent(user.mailId, newPassword);
  ...
  async function handleSendOtp() {
    try {
      setSendingOtp(true);
      const response = await requestOtpStudent(user.mailId);
      setSendingOtp(false);
      if (response.statusCode === 200) {
        dispatch(
          uiActions.showSuccessNotification({
  ...
  async function validateOtpAction(prev, formData) {
    ...
    try {
      const response = await validateOtpStudent(user.mailId, otp);
      if (response.statusCode === 200) {
        dispatch(
          uiActions.showSuccessNotification({
            status: "success",
  ...
  return (
    <div className="p-6 bg-gray-100 rounded-md shadow-md max-w-lg mx-auto my-8 py-6">
